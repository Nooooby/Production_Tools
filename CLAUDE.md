...
**运行 Dashboard 增强脚本**:
```bash
python automation/enhance_dashboard.py
```

**运行生产规划表创建脚本**:
```bash
python automation/create_production_planning_v2.py
```

**运行物料转换逻辑修复脚本**:
```bash
# 修复订单表转换逻辑
python automation/fix_conversion_logic.py

# 修复 BulkPack 和 Bagging
python automation/fix_bulkpack_bagging.py

# 修复 Cages 聚合逻辑
python automation/fix_cages_max_logic.py

# 修复 #REF! 错误
python automation/fix_ref_error_planning.py
```

**Windows 任务计划集成**:
```powershell
# 查看设置脚本（需根据环境修改）
cat automation\setup_task_scheduler.ps1

# 运行批处理文件（手动执行）
automation\schedule_daily_report.bat
```
...
│   ├── daily_report_automation.py       # 日报生成 (370 行)
│   ├── enhance_dashboard.py             # Dashboard 增强 (438 行)
│   ├── create_production_planning_v2.py # 生产规划 (375 行)
│   ├── fix_conversion_logic.py          # 订单转换逻辑修复 (229 行)
│   ├── fix_bulkpack_bagging.py          # BulkPack/Bagging 修复 (276 行)
│   ├── fix_cages_max_logic.py           # Cages 聚合修复 (151 行)
│   ├── fix_ref_error_planning.py        # #REF! 错误修复 (230 行)
│   ├── config_email.json                # 邮件配置模板
│   ├── schedule_daily_report.bat        # Windows 批处理
│   ├── setup_task_scheduler.ps1         # PowerShell 任务计划脚本
│   └── requirements.txt                 # Python 依赖
├── data/                    # Excel 数据文件
│   ├── v39_Dashboard_Enhanced.xlsx      # ⭐ 最新工作版本
│   ├── Production Operations Dashboard v38.xlsx  # 原始版本
│   └── v39_*.xlsx          # 各版本备份
├── logs/                    # 执行日志
│   └── daily_report_*.log
├── docs/                    # 项目文档
...
本项目围绕**生产物料转换**这一核心业务逻辑：

```
订单输入 (Cases)
    ↓ × 产品平均重量 (Avg_Case_Weight)
WIP 需求 (kg)
    ↓ ÷ 产率百分比 (Yield%)
原始肉类需求 (kg)
    ↓ ÷ 680 kg/cage
...
这个转换链应用于三种订单类型：
- **TrayPack** (05_Daily_Orders 表): 采购商订单
...
1. **主数据层** (蓝色标签):
   - `00_SKU_Master`: 产品主数据 (1,315+ 处引用)
   - `00_Yield_Rates`: 产率数据 (按产品组分类)

2. **订单输入层** (绿色标签):
   - `01_Cages_Plan`: 鸡笼库存和计划
   - `02_TrayPack_Order`: 采购订单
   - `03_BulkPack_Order`: 散装订单
   - `04_Bagging_Order`: 装袋订单
   - `05_Daily_Orders`: 当日订单汇总 (含物料转换逻辑)
   - `06_Resource_Plan`: 原料计划 (42 种原料)
...
4. **分析输出层** (紫色标签):
   ...
   - `14_Production_Planning`: 生产规划和 Cages 汇总
...
#### 产品分类（00_SKU_Master）
- 产品按 `Product_Group` 分类: BSB, ThighMeat, Drum, Wing, SplitBreast 等
- 每种产品有 `Avg_Case_Weight` (平均 case 重量)
- 总计 100+ 种产品 SKU

#### 产率数据（00_Yield_Rates）
- 根据产品组定义产率百分比
- 示例: BSB 31%, ThighMeat 10.5%, Drum 12.8%, Wing 11%
- 用于计算从原始肉类到成品的转换
...
1. **`fix_conversion_logic.py`**: 给订单表添加物料转换公式
   - 为 05_Daily_Orders 添加 N-S 列 (1,962 个公式)
   - 为 10_Cone_Line 添加 I-N 列 (768 个公式)
   - 为 04_Bagging_Order 添加 N-S 列 (108 个公式)
   - 输出: `v39_Dashboard_Enhanced.xlsx`

2. **`fix_cages_max_logic.py`**: 修正 Cages 聚合逻辑
   - 在 14_Production_Planning 添加按产品组汇总 (Rows 22-33)
   - 计算 MAX(所有产品组) 而非 SUM
   - 原理: 切一个笼同时产出所有产品

3. **`enhance_dashboard.py`**: 增强执行仪表板
   - 添加 6 个 KPI 指标卡片 (O2:P13)
   - 创建 3 个专业可视化图表
   - 应用 3 种条件格式规则 (着色)

4. **`daily_report_automation.py`**: 生成每日报告
   - 从当前 Excel 提取数据
   - 计算订单完成率、产量统计
   - 生成日报文件并发送邮件
...
**添加新订单**:
1. 在 `05_Daily_Orders` 表输入订单 (B 列 SKU, M 列 Cases)
2. 脚本自动计算:
   - N 列: Product_Group (XLOOKUP)
   - O 列: Avg_Case_Weight (XLOOKUP)
   - P 列: Yield_Rate (XLOOKUP)
   - Q 列: WIP_kg = M × O
   - R 列: Raw_kg = Q ÷ P
   - S 列: Cages = R ÷ 680
3. 自动聚合到 `14_Production_Planning` 计算所需鸡笼数

**生成日报**:
1. 运行 `daily_report_automation.py`
2. 自动读取当日订单、计算完成率
3. 生成 HTML 邮件和 Excel 日报文件
4. 所有操作记录到日志
...
**问题: Excel 文件显示 #REF! 错误**
- 原因: 列引用被删除或重命名
- 解决: 运行对应的修复脚本 (例如 `fix_ref_error_planning.py`)
- 验证: 检查源表是否存在且列名正确

**问题: #VALUE! 错误**
- 原因: 公式中的数据类型不匹配
- 解决:
  1. 检查 XLOOKUP 的返回值是否为预期类型
  2. 验证数值列是否被误作为文本
  3. 运行数据验证脚本
...
- 原因: 查找值在目标表中不存在
- 解决:
  1. 验证 SKU 在 00_SKU_Master 中存在
  2. 检查 Product_Group 在 00_Yield_Rates 中存在
  3. 查看是否有拼写或大小写差异

**问题: Python 脚本运行失败**
- 检查 Python 依赖是否已安装: `pip list | findstr openpyxl`
- 验证文件路径是否存在和可访问
- 查看 logs/ 目录中的错误日志
- 确保 Excel 文件未被其他程序打开

**问题: 日报邮件未发送**
- 检查 config_email.json 的邮箱配置
- 验证网络连接
- 查看日志文件获取详细错误信息
- 确认 SMTP 服务器地址和端口正确
...
所有修复脚本都会自动创建备份文件：
```
v39_Dashboard_Enhanced_backup_before_[script_name]_[timestamp].xlsx
```

示例:
- `v39_Dashboard_Enhanced_backup_before_fix_20260102_065058.xlsx`
- `v39_Dashboard_Enhanced_backup_bulkpack_20260102_065653.xlsx`

建议保留最近的 3-5 个备份，定期清理旧版本。

### 性能优化建议

1. **减少行数**: 只保留活跃订单，归档历史数据到单独的工作簿
2. **删除未使用列**: 定期审计并删除不需要的列
3. **优化图表**: 限制数据范围而非全列引用
4. **使用 Excel 表格**: 将范围转换为命名表，提高查询效率
...
- **日报流程**: `daily_report_automation.py:DailyReportGenerator.run()`
- **物料转换**: `05_Daily_Orders!N-S` 列公式
- **Cages 聚合**: `14_Production_Planning!E34` 公式
- **仪表板**: `12_Executive_Dash` 工作表 (KPI O2:P13)
...
*Last Updated: 2026-01-02 18:30* (CLAUDE.md 完成 - 添加开发工作流、系统架构和调试指南)
